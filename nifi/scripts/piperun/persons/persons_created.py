# scripts para migrar dados criados no dia das pessoas do piperun para o redshift 
try:
    import os
    import sys
    import json
    ...
    url_opt = "https://api.pipe.run/v1/persons?created_at_start="+day_1+"&show=200&page={}"

    opts    = piperun().get(url_opt)
    records = persons.records(opts)

    day = (datetime.now() + timedelta(days=-2,hours=-3)).strftime('%Y-%m-%d') + ' 23:59:59'
    pg("polichat").exec_pgsql("delete from piperun.persons where created_at >'{}'".format(day))
