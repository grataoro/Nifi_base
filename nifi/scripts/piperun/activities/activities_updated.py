try:
    ...
    url = "https://api.pipe.run/v1/activities?updated_at_start="+day_1+" 00:00:00&with=pipeline,stage,involved&show=200&page={}"

    data = piperun().get(url)
    records = atividades.records(data)
    atv_ids = piperun.ids(records)

    day = (datetime.now() + timedelta(days=-2,hours=-3)).strftime('%Y-%m-%d') + ' 23:59:59'
    pg("polichat").exec_pgsql("delete from piperun.activities where id in {}".format(atv_ids))
