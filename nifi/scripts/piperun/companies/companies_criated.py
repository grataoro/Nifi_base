try:
    ...
    url = "https://api.pipe.run/v1/companies?created_at_start="+day_1+"&show=200&page={}"

    data = piperun().get(url)
    records = empresas.records(data)

    day = (datetime.now() + timedelta(days=-2,hours=-3)).strftime('%Y-%m-%d') + ' 23:59:59'
    pg("polichat").exec_pgsql("delete from piperun.companies where created_at >'{}'".format(day))
